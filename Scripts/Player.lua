...
function Player.server_updateGameState( self, State, caller )
	...
	if self.state == States.Play then
...
function Player.client_setlockInteractable(self, shape)
	self.lockingShape = shape
	self.player.character:setLockingInteractable( shape.interactable )
end